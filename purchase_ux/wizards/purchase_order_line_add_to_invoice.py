...
class PurchaseOrderLineAddToInvoice(models.TransientModel):
    ...
    def get_partner_id(self):
        ...
        if len(partner) != 1:
            raise UserError(_("Selected lines must be from the same partner"))
        return partner[0].id
    ...
    def confirm(self):
        ...
        pol.with_context(
            do_not_compute=True, voucher=self.voucher, active_id=self.invoice_id.id, active_model="account.move"
        ).action_add_all_to_invoice()

...
class TestPurchaseOrderTypeLock(PurchaseTestCommon):
    ...
    def test_01_lock_on_confirmation_type_flag(self):
        """AC2: tipo con flag ON + global OFF -> la orden queda bloqueada (locked)."""
        po = self._create_po(self.type_lock)
        po.button_confirm()
        self.assertEqual(po.state, "purchase")
        self.assertTrue(po.locked, "La OC de un tipo con 'Lock on Confirmation' debe quedar bloqueada")

    def test_02_no_lock_without_flag(self):
        """AC3: tipo con flag OFF + global OFF -> la orden queda editable (sin bloquear)."""
        po = self._create_po(self.type_no_lock)
        po.button_confirm()
        self.assertEqual(po.state, "purchase")
        self.assertFalse(po.locked, "Sin flag y sin lock global la OC debe quedar editable")
    ...
    def test_03_global_lock_dominates(self):
        ...
        po = self._create_po(self.type_no_lock)
        po.button_confirm()
        self.assertTrue(po.locked, "Con el lock global prendido la OC debe quedar bloqueada igual")
    ...
    def test_04_double_validation_respected(self):
        ...
        self.assertFalse(po.locked, "Pendiente de aprobación no debe bloquearse todavía")
        # Aprobación final (usuario manager) -> recién ahora bloquea.
        po.button_approve()
        self.assertEqual(po.state, "purchase")
        self.assertTrue(po.locked, "Tras la aprobación final la OC del tipo con flag debe bloquearse")
